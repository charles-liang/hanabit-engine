...
local third_party_dir = src_dir .. "/third-party"
...
target(target_name)
    ...
    elseif is_mode("release") then
     ...
     -- or "shared" if you need a dynamic library
    set_languages("cxx20")
...
target(target_name .. "_test")
    ...
    add_packages("gtest") -- 添加 gtest 和 benchmark 包
    add_deps(target_name)  -- Add dependency on the core library
    common_target_config()
    set_targetdir("$(buildir)/bin/$(plat)/$(arch)/$(mode)")

    -- 目标: 基准测试
target(target_name .. "_benchmark")
    set_kind("binary")
    set_languages("cxx20")
    add_files("benchmarks/*.cpp")  -- 添加基准测试文件
    add_files("benchmarks/benchmarks_main.cpp")  -- 添加用于基准测试的 main 函数
    add_packages("benchmark") -- 只需要 benchmark 包
    add_deps(target_name)  -- 添加对核心库的依赖
    common_target_config()
    set_targetdir("$(buildir)/bin/$(plat)/$(arch)/$(mode)")